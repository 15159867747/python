...
class Handler(BaseHandler):
    ...
    def detail_page(self, response):
        ...
        wymm=re.search('<td class="t_f".*?微云：.*?<a href=.*?target="_blank">.*?密码：(.*?)<br', response.text,re.S)
        ...
        jymm=re.search('<td class="t_f".*?解压密码：(.*?)<br />', response.text,re.S)
