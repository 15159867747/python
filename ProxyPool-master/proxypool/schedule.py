...
class ValidityTester(object):
    ...
    def set_raw_proxies(self, proxies):
        self._raw_proxies = proxies #待检测的数组
    ...
    async def test_single_proxy(self, proxy):#异步检测
        ...
        """
        try:
            async with aiohttp.ClientSession() as session: #异步请求库
                try:
...
class PoolAdder(object):
    ...
    def __init__(self, threshold):
        ...
        self._crawler = FreeProxyGetter()#代理抓取
...
class Schedule(object):
    ...
    def valid_proxy(cycle=VALID_CHECK_CYCLE):
        """
        定时检查
        VALID_CHECK_CYCLE  时间参数
        Get half of proxies which in redis
        """
        conn = RedisClient()#数据库连接对象
        tester = ValidityTester()#检查代理是否可用的类
        while True:
            print('Refreshing ip')
